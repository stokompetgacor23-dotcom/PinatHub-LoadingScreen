-- ==================================================
-- SCRIPT: ZeOrbitV4 Game Hub (WINDUI v2)
-- ULTRA OPTIMIZED + NO FREEZE
...
-- CREATE TABS
-- =======================================================
local OrbitTab = Window:Tab({ Title = "Orbit", Icon = "orbit" })
local FlingTab = Window:Tab({ Title = "Fling", Icon = "wind" })
local ToolsTab = Window:Tab({ Title = "Tools", Icon = "settings" })
local KillTab = Window:Tab({ Title = "Kill", Icon = "skull" })
local PlaceTab = Window:Tab({ Title = "Place", Icon = "map-pin" })
local SkinTab = Window:Tab({ Title = "Skin", Icon = "palette" })
...
-- Smoothing untuk zoom dan update kamera
...
-- ORBIT TAB
local orbitMainSection = OrbitTab:Section({ Title = "Player Controls" })
...
OrbitTab:Space()
local orbitSettingsSection = OrbitTab:Section({ Title = "Orbit Settings" })
...
-- FLING TAB
local flingMainSection = FlingTab:Section({ Title = "Select Player & Fling" })
...
FlingTab:Space()
local flingAllSection = FlingTab:Section({ Title = "Mass Fling" })
...
FlingTab:Space()
local touchFlingSection = FlingTab:Section({ Title = "Touch Fling" })
...
-- KILL TAB
local killMainSection = KillTab:Section({ Title = "Instant Kill System" })
...
-- TOOLS TAB
local toolsMainSection = ToolsTab:Section({ Title = "Auto Features" })
...
-- PLACE TAB
local placeMainSection = PlaceTab:Section({ Title = "Place Teleport System" })
...
PlaceTab:Space()
local placeSelectionSection = PlaceTab:Section({ Title = "Select Place to Teleport" })
...
PlaceTab:Space()
local backToPlaceSection = PlaceTab:Section({ Title = "Return Functions" })
...
-- SKIN TAB
local skinMainSection = SkinTab:Section({ Title = "Skin Modifier" })
...
-- SERVER TAB
local serverMainSection = ServerTab:Section({ Title = "Server Information" })
...
task.spawn(function()
    while true do
        serverInfoParagraph:SetDesc("Players: " .. tostring(#Players:GetPlayers()) .. "/20")
        task.wait(10)
    end
end)

-- COMMUNITY TAB
local communityMainSection = CommunityTab:Section({ Title = "Community Links" })
...
CommunityTab:Space()
local communityLinksSection = CommunityTab:Section({ Title = "Available Platforms" })
